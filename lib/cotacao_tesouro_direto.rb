...
module CotacaoTesouroDireto
	...
	def self.parse_response(body)
		...
		doc.xpath("#{MAIN_XPATH}td[@class='listing2']").remove #remove a linha que contém a data de atualização da cotação
	...
	def self.format_response(data)
		...
		data[2] == "-" ? data[2] = nil : data[2] = data[2].to_delocalized_decimal.to_f
		data[3] == "-" ? data[3] = nil : data[3] = data[3].to_delocalized_decimal.to_f
		data[4] == "-" ? data[4] = nil : data[4] = data[4].to_delocalized_decimal.gsub("R$ ", "").to_f
		data[5] == "-" ? data[5] = nil : data[5] = data[5].to_delocalized_decimal.gsub("R$ ", "").to_f
	...
	def self.to_hash(data)
		response_hash = {}
		response_hash[:updated_at] = @@updated_at.to_s
